...
<div class="container">
    ...
    <ul class="nav status-tabs flex-wrap mb-4">
        <li class="nav-item"><button class="nav-link active bg-secondary text-white" data-bs-toggle="tab" data-bs-target="#semua">All 📋</button></li>
        <li class="nav-item"><button class="nav-link bg-warning text-dark" data-bs-toggle="tab" data-bs-target="#pending">Pending ⏳</button></li>
        <li class="nav-item"><button class="nav-link bg-info text-white" data-bs-toggle="tab" data-bs-target="#proses">Proses ⚙️</button></li>
        <li class="nav-item"><button class="nav-link bg-success text-white" data-bs-toggle="tab" data-bs-target="#selesai">Selesai ✅</button></li>
        <li class="nav-item"><button class="nav-link bg-danger text-white" data-bs-toggle="tab" data-bs-target="#tolak">Ditolak ❌</button></li>
    </ul>
            ...
            @forelse($pengaduan->sortByDesc('created_at') as $p)
                <div class="pengaduan-item d-flex justify-content-between p-3 mb-3 shadow-sm rounded-4 bg-white">
                    <div class="d-flex align-items-start gap-3">
                        {{-- Gambar + Deskripsi --}}
                        <img src="{{ $p->bukti ? asset('storage/'.$p->bukti) : asset('img/noimage.png') }}" alt="bukti" class="rounded" style="width:90px; height:90px; object-fit:cover;">
                        <div>
                    ...
                    <div class="text-end d-flex flex-column justify-content-between" style="min-width:120px;">
                        ...
                        <a href="{{ route('user.tanggapan.show', $p->id) }}" class="btn btn-info btn-sm rounded-pill w-100">detail 🔍</a>
                    </div>
                </div>
            @empty
...
{{-- Style --}}
<style>
.status-tabs .nav-link { border:none; border-radius:20px; font-weight:600; margin-right:5px; padding:6px 14px; transition:0.3s; }
.badge-pending { background-color:#ffc107; }
.badge-proses { background-color:#0dcaf0; }
.badge-selesai { background-color:#28a745; }
.badge-tolak { background-color:#dc3545; }
.pengaduan-item { transition:0.3s ease; }
</style>
